...
class PlotterCallback(Callback):
    ...
    def get_label_predictions(self, label_force, predicted_force):
    ...
    def write_graph(self, epoch, train_labels, train_predictions, val_labels, val_predictions):
        ...
        train_predicted_force = self.get_force(train_features, train_predictions, feature_len=self.train_generator.feature_len)
        train_label_force = self.get_force(train_features, train_labels, feature_len=self.train_generator.feature_len)

        val_predicted_force = self.get_force(val_features, val_predictions, feature_len=self.val_generator.feature_len)
        val_label_force = self.get_force(val_features, val_labels, feature_len=self.val_generator.feature_len)

        train_label_force, train_predicted_force = self.get_label_predictions(train_label_force, train_predicted_force)
        val_label_force, val_predicted_force = self.get_label_predictions(val_label_force, val_predicted_force)


        loss_ax.plot(epochs, train_loss, 'g', label='train')
        loss_ax.plot(epochs, val_loss, 'r', label='val')

        train_ax.plot(train_label_force, 'g', label='label')
        train_ax.plot(train_predicted_force, 'r', alpha=0.2, label='prediction')
        val_ax.plot(val_label_force, 'g', label='label')
        val_ax.plot(val_predicted_force, 'r', alpha=0.2, label='prediction')
